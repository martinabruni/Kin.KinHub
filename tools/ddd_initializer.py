...
CS_PROJ_TEMPLATE = """<Project Sdk="Microsoft.NET.Sdk">
  <PropertyGroup>
...
def write_csproj(path: Path, project_name: str, tfm: str) -> Path:
    path.parent.mkdir(parents=True, exist_ok=True)
    if not path.exists():
        path.write_text(
            CS_PROJ_TEMPLATE.format(name=project_name, tfm=tfm), encoding="utf-8"
        )
    return path
...
def add_project(
    ...
    return write_csproj(project_path, project_name, tfm)
